...
class AgentDrawer(ft.NavigationDrawer):
    ...
    def update_agents(self):
        ...
        self.controls = [
            ...
            ft.NavigationDrawerDestination(
                icon=ft.Icon(ft.Icons.ADD_ROUNDED),
    ...
    async def agent_change(self, e):
        ...
        if selected.label == 'Initialize new wallet':
            self.page.show_dialog(self.agent_init)
            ...
            agent_connection = agenting.AgentConnection(self.app, self.page, self.config, selected.label)
            self.page.show_dialog(agent_connection)
